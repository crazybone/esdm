...
function App() {
  ...
  useEffect(() => {
    console.log('bannerType 0: ', bannerType);
    if (data.departments.length > 0) {
      ...
      setBannerType(bannerType || 1);
      setSelectedBanner(defaultBanner || null);
      if(defaultBanner) {        
        let bContent = defaultBanner.content;
        setBannerContent(bContent || '');
        //console.log('bContent 0: ', bContent);
      }
    }
  }, [data, bannerType]);    
                ...
                <Accordion key={section.id} sx={{ marginTop: 2 }}>
                  ...
                  <AccordionDetails key={section.banner}>
                    <Select
                      ...
                      value={bannerType}
                      onChange={(event) => {
                        setBannerContent(handleDisplayBanner(dept.id, section.id, event.target.value) || '');                        
                      }}
                      fullWidth
                    >
                      {section.banner.map((b) => (
                        <MenuItem key={b.type} value={b.bannerid}>{b.type} Column</MenuItem>
                      ))} 
                    </Select>

                    {selectedBanner && (
                      <Box id={selectedBanner.name.replace(/\s+/g, "_")} dangerouslySetInnerHTML={{ __html: bannerContent }} sx={{ marginTop: 2, maxWidth: '640px', height: '100px', display: 'inline-flex', justifyContent: 'space-between' }}>
                      </Box>         
                    )}
